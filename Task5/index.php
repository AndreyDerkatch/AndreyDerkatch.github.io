...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
  ...
  if (!empty($_COOKIE[session_name()]) && session_start() && !empty($_SESSION['login']))
  {
    ...
    $user = 'u52881'; 
    $pass = '1598062';
    $db = new PDO ("mysql:host=localhost;dbname=u52881", $user, $pass, [PDO::ATTR_PERSISTENT => true, PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION]);
    $id1=$_SESSION["uid"];
    $data = $db->query("SELECT * from ".'DATA'." where id=$id1")->fetch();
    ...
    $data = $db->query("SELECT * from DATA_LINK_SUPERPOWER where id_DATA_FORM=$id1")->fetchAll();
    if(!empty($data)){
    if(is_array($data[0]))
      foreach($data as $d){
        switch($d['id_DATA_superpower']) {
      ...
      switch($data['id_DATA_superpower']) {
        case 1: {$values['sg']='selected'; setcookie('sg_value', 'selected', time() + 30 * 24 * 60 * 60); break;}
        case 3: {$values['sl']='selected'; setcookie('sl_value', 'selected', time() + 30 * 24 * 60 * 60); break;}
        case 2: {$values['sw']='selected'; setcookie('sw_value', 'selected', time() + 30 * 24 * 60 * 60); break;}
      }
...
// Иначе, если запрос был методом POST, т.е. нужно проверить данные и сохранить их в XML-файл.
  ...
  // Проверяем меняются ли ранее сохраненные данные или отправляются новые.
    ...
    $stmt = $db->prepare("UPDATE ".'DATA'." set name= :fname, email= '". $_POST['email'] ."' ,bdate= :bdate, gender= :gender,kol= :kol,biography= :biography where id=$id1");
    ...
    $stmt=$db->prepare("DELETE from DATA_LINK_SUPERPOWER where id_DATA_FORM=$id1");
    $stmt->execute();
    ...
    foreach ($_POST['super'] as $s) {
      switch($s) {
        case "Бессмертие": {
          $stmt = $db->prepare("INSERT INTO DATA_LINK_SUPERPOWER VALUES($id1, 1)");
          $stmt->execute();; break;
        }
        case "Левитация": {
          $stmt = $db->prepare("INSERT INTO DATA_LINK_SUPERPOWER VALUES($id1, 3)");
          $stmt->execute(); break;
        }
        case "Прохождение сковзь стены": { 
          $stmt = $db->prepare("INSERT INTO DATA_LINK_SUPERPOWER VALUES($id1, 2)");
          $stmt->execute(); break;
        }
      }
    }
    ...
    $pass = '1598062';
    $db = new PDO ("mysql:host=localhost;dbname=u52881", $user, $pass, [PDO::ATTR_PERSISTENT => true, PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION]);
    $stmt = $db->prepare("INSERT INTO ".'DATA'." VALUES(NULL, :fname, :email ,:bdate,:gender,:limb,:biography)");
