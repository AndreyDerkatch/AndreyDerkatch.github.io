...
if($_SERVER['REQUEST_METHOD']=='GET') {
    ...
    if($stmt['pass']!=md5($pw)){
      ...
      if(is_array($data)){
        ...
        for($i=0;$i<$count;$i++ ){
          ...
          $sw='';
          $sl='';
          $super=$db->query("SELECT * from DATA_LINK_SUPERPOWER where id_DATA_FORM=$id")->fetchAll();
          if(is_array($super))
            foreach($super as $s)
              switch($s['id_DATA_superpower']){
                case 1: {
                  $sg='selected'; break;
                }
                case 2: {
            ...
            switch($super['id_DATA_superpower']){
              case 1: {
                $sg='selected'; break;
              }
              case 2: {
          ...
          print('<tr>');
          print("<td> <input type='checkbox' name='ch$i' value='$id' style='height:25px; width: 25px;'>   </td>");
          print("<td>". strval($id)."</label>");
          print("<td> <input name='name$i' class='$name_err' value='$name'>  </td>");
          print("<td> <input name='email$i' class='$email_err' value='$email'>  </td>");
          print("<td> <input type='date' class='$bdate_err' name='bdate$i' value='$bdate'> </td>");
          print("<td> <label class='$gender_err'>М<input type='radio'  name='gender$i' value='M' $mg>  Ж:<input type='radio' name='gender$i' value='W' $wg> </label> </td>");
          print("<td> <div class='$limb_err'>");
          for($j=1;$j<=4;$j++){
            if($j==($kol)){
          ...
          print("</div> </td>");
          print("<td> <select class='$super_err' name='super$i".'[]'."' multiple> <option name='super1' value='Бессмертие' $sg>Бессмертие</option> <option name='super2' value='Прохождение сковзь стены' $sw>Прохождение сковзь стены</option> <option name='super3' value='Левитация' $sl>Левитация</option> </select> </td> ");
          print("<td> <textarea class='$biography_err' name='biography$i'>$biography</textarea> </td>");
          print('</tr>');
  ...
  print("<div style='display: flex; flex-direction: row;'> <input class='submit' type='submit' name='change' value='Изменить выбранные'> ");
  print("<input class='submit' type='submit' style='margin-left:10px; margin-right:10px;' name='delete' value='Удалить выбранные'> ");
  ...
  try{
    $db=db();
    $count = $db->query("SELECT id from ".'DATA')->fetchAll();
    $count = count($count);
    $onchange=array();
    for($i=0;$i<$count;$i++){
      if(!empty($_POST['ch'.$i])){
    ...
    if(!empty($_POST['change'])){
      foreach($onchange as $s){
        ...
        if($err){
          ...
          $stmt = $db->prepare("UPDATE ".'DATA'." set name= :fname, email=:email ,bdate= :bdate, gender= :gender, kol= :kol,biography= :biography where id=:id");
          ...
          $stmt->execute();
          $id=$_POST['ch'.$s];
          $stmt = $db->prepare("DELETE from DATA_LINK_SUPERPOWER where id_DATA_FORM=$id");
          $stmt->execute();
          foreach ($_POST['super'.$s] as $sup) {
            switch($sup) {
              case "Бессмертие": {
                $stmt = $db->prepare("INSERT INTO DATA_LINK_SUPERPOWER VALUES($id, 1)");
                $stmt->execute();; break;
              }
              case "Левитация": {
                $stmt = $db->prepare("INSERT INTO DATA_LINK_SUPERPOWER VALUES($id, 3)");
                $stmt->execute(); break;
              }
              case "Прохождение сковзь стены": { 
                $stmt = $db->prepare("INSERT INTO DATA_LINK_SUPERPOWER VALUES($id, 2)");
                $stmt->execute(); break;
              }
            }
          }
